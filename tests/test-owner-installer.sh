...
for payload in \
  ...
  'scripts/offgridpi_owner_auth.py' \
...
grep -qF \
  'python3 -m py_compile /opt/offgridpi/scripts/offgridpi_owner_auth.py' \
  "$INSTALLER" ||
  fail "Installer does not syntax-check the Owner authentication module."
